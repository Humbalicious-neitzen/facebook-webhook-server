...
4. analyzeImage():
   - Use this tool ONLY when the 'imageUrl' is present in the user content.
   ...
   - If the tool returns an error saying the text is unclear, say so briefly and ask for clarification.
...
async function askBrain({ text, imageUrl, surface, history = [] }) {
  ...
  try {
    ...
    // We will ask for JSON response format right away if no tools are likely to be called.
    ...
    // If there's no image and the text is simple, we can be more efficient
    // by asking for the JSON format directly. For complex queries, let the model decide.
    const isSimpleQuery = !imageUrl && !text.includes("weather") && !text.includes("route") && !text.includes("nearby");
    if (isSimpleQuery) {
    ...
    if (!responseMessage.tool_calls) {
      ...
      // If we didn't request JSON initially, we do it now.
      const finalPayload = {
        model: MODEL,
    ...
    for (const toolCall of toolCalls) {
      ...
      if (funcToCall) {
        let result;
        if (funcName === 'analyzeImage') {
          // Ensure analyzeImage is only called if an imageUrl is actually present.
          if (imageUrl) {
            result = await funcToCall({ imageUrl, ...funcArgs });
          } else {
            result = { error: "The user did not provide an image to analyze." };
          }
        } else {
          result = await funcToCall(funcArgs);
        }
...
function finalizeResponse(parsed, surface) {
  ...
  if (surface === "dm" && parsed.message && parsed.message.includes("BEGIN_CARD") && parsed.message.includes("END_CARD")) {
...
async function analyzeImage({ imageUrl }) {
  if (!imageUrl) {
    // This case should ideally not be hit if called correctly from askBrain
    return { error: "No image URL was provided to the analyzer." };
  }

  try {
    // Step 1: Download the image content into a buffer
    ...
    // Step 2: Process the image buffer with Tesseract
    const { data: { text } } = await Tesseract.recognize(imageBuffer, 'eng+urd', {
      logger: m => console.log(`Tesseract: ${m.status} (${(m.progress * 100).toFixed(0)}%)`)
    });

    if (!text || text.trim().length < 5) {
        return { error: "The image was analyzed, but no meaningful text was found. It might be too blurry or contain no text." };
    }

    return { extracted_text: text.trim() };

  } catch (e) {
    console.error("Error in analyzeImage:", e.message);
    // Provide a more specific error based on the type of error
    if (e.isAxiosError) {
      return { error: "There was an issue downloading the image. The link might be broken or expired." };
    }
    // Tesseract-specific errors or other issues
    return { error: "Could not analyze the image. It might be in an unsupported format, or the content is unreadable." };
  }
}
